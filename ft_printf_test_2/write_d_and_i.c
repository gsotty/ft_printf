...
/*   Updated: 2017/02/14 08:41:23 by gsotty           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*ft_flag_1(t_struc *struc, char *tmp, t_len *len)
{
	if (struc->flag.negatif == 1)
		tmp = ft_flag(tmp, len, "-", 1);
...
static char	*ft_if_precision(t_struc *struc, char *tmp, t_len *len)
{
	...
	if (struc->precision.number > len->len_tmp)
	{
		...
		tmp = ft_flag(tmp, len, tmp_prec, struc->precision.number -
				len->len_tmp);
	}
	tmp = ft_flag_1(struc, tmp, len);
...
int			write_d_and_i(t_struc *struc, char **buf, t_len *len, va_list ap)
{
	...
	if (struc->flag.nbr_zero == 0 && struc->precision.number != -1
